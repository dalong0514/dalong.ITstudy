...
先安装专门为 mac 的 m 芯片配套的 torch 包：

uv pip install --pre torch torchvision torchaudio --extra-index-url https://download.pytorch.org/whl/nightly/cpu

经过核查，目前 Mac 上安装 PyTorch 时无需 nightly 版了，跳过上面步骤。

[PyTorch on Apple Silicon - Fabrizio Musacchio](https://www.fabriziomusacchio.com/blog/2024-03-16-pytorch_on_apple_silicon/?utm_source=chatgpt.com)

安装 mac 上实现量化/加速方案的 mlx 包：

uv pip install mlx-lm

如果 GitHub 项目里有现成的 equirements.txt 那么直接安装文件内的 Python 包。

uv pip install -r requirements.txt

导出新版的 requirements.txt

uv pip freeze > requirements.txt

03 将 Python 包信息写入文件 `pyproject.toml`
...
1 在项目根目录生成锁文件（基于 pyproject.toml 或 requirements）
uv lock

2 严格按锁文件安装（本地或 CI）
uv sync --locked
等价：uv pip install --locked（根据你的工作流可能也会见到）

3 升级某个包并刷新锁
uv lock --upgrade transformers
...
没办法直接通过 `uv pip install -r requirements.txt` 下面几个 mac 芯片优化的 torch 包。

```
torch==2.10.0.dev20251009
torchaudio==2.8.0.dev20251009
torchvision==0.25.0.dev20251009
```
...
hf download --resume-download --local-dir-use-symlinks False deepseek-ai/DeepSeek-OCR --local-dir DeepSeek-OCR

huggingface-cli download --resume-download --local-dir-use-symlinks False deepseek-ai/DeepSeek-OCR --local-dir DeepSeek-OCR
