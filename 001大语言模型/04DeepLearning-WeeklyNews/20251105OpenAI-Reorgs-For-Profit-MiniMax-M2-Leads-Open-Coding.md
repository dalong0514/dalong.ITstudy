20251105OpenAI-Reorgs-For-Profit-MiniMax-M2-Leads-Open-Coding
...
如今，AI 智能体在分析企业内各类数据、发现规律并创造价值方面，能力越来越强。而这恰恰让数据孤岛的问题显得愈发棘手。正因如此，我现在越来越倾向于选择那些能让我掌控自己数据的软件，这样我就能让我的 AI 智能体充分利用这些数据。
...
随着人工智能（AI）能力的日益增强，通过在不同数据片段之间「连接点」所能创造的价值，如今已达到了前所未有的高度。举例来说，假设一个电子邮件点击事件记录在供应商 A 的系统中，而后续的在线购买行为却记录在供应商 B 的系统里。那么，构建能够同时访问这两个数据源的 AI 智能体，来分析它们之间的关联性，从而辅助做出更优的决策，就显得极具价值。
...
遗憾的是，许多 SaaS（软件即服务）服务商试图在其客户业务中制造数据孤岛。通过增加您提取自身数据的难度，他们制造了高昂的转换成本。这也能促使您购买他们的 AI 智能体服务（这些服务有时价格不菲且 / 或质量不佳），而不是自行构建或从其他服务商处购买。更糟糕的是，一些 SaaS 服务商察觉到 AI 智能体正试图获取这些数据，因此正设法让您（以及您的 AI 智能体）更难高效地访问它们。
...
我手下的一个团队刚刚告诉我，我们一直用于存储客户数据的一家 SaaS（软件即服务）供应商，现在竟然要收取超过 2 万美元，才肯给我们一个访问自身数据的 API 密钥。这种高得离谱的收费 —— 其设计意图无疑就是为了增加客户取出数据的难度 —— 正在阻碍我们实施那些旨在利用这些数据的 AI 智能体（AI Agent）工作流。
...
我通过 AI Aspire（一家人工智能咨询公司）间或为企业提供 AI 战略咨询。在采购 SaaS（软件即服务）时，我常建议企业要设法掌控自身的数据（可惜的是，一些供应商对此极力反对）。如此一来，你可以选用某个 SaaS 供应商来记录并在其平台上管理你的数据，但数据的最终流向 —— 是交由合适的人员还是 AI 系统来处理 —— 这个决定权应掌握在你手中。
...
过去十年间，业界投入了大量精力来管理企业的结构化数据。由于如今人工智能处理非结构化数据的能力远超以往，因此，整理你的非结构化数据（包括 PDF 文件 —— 这正是 LandingAI 的智能文档提取（Agentic Document Extraction）所专精的领域）变得比以往任何时候都更有价值。

In the era of generative AI, businesses and individuals have important work ahead to organize their data to be AI-ready.

在生成式 AI（Generative AI）的时代，企业和个人都面临着一项重要任务：整理数据，为 AI 应用做好准备。
...
OpenAI 完成了从非营利组织向营利性组织的转型。这项复杂的法律架构调整历时 18 个月才得以敲定，其间动用了大批律师、投资银行家，并经过了两位州检察长的谈判。
...
最新动态：重组后的 OpenAI Group PBC 是一家公益公司（Public Benefit Corporation），这是一种以创造积极社会影响为使命的营利性企业。它可以为投资者带来无上限的回报，从而为吸引更多资金（包括未来可能的首次公开募股）铺平了道路。该公司仍受一家非营利基金会 —— 新更名的 OpenAI 基金会 —— 的监督，该基金会持有集团 26% 的股份。根据双方新的合作伙伴关系条款，微软持有 OpenAI 27% 的股份。
...
具体来说：这项协议让 OpenAI 不再受限于其 2015 年作为非营利组织创立时的框架。自 2019 年早些时候的一次重组以来，该框架一直将投资者的最高回报限制在 100 倍以内。新的架构旨在回应加利福尼亚州和特拉华州监管官员的关切，他们认为旧架构在服务公众与回报股东之间产生了利益冲突。与此同时，新架构也力求延续公司的核心使命：确保通用人工智能（AGI）—— 如果 OpenAI 未来成功开发出它 —— 将造福全人类。
...
作为一家公益型企业（Public Benefit Corporation，PBC），OpenAI 必须在盈利增长与履行社会公益之间取得平衡。在 AI 公司中，Anthropic 和 GrokAI 同样采用了公益型企业的架构。
...
OpenAI 的组织结构依然独特，因为从法律架构上讲，最终控制权仍掌握在一个非营利组织手中。OpenAI 基金会拥有任命和罢免公司董事会成员的权力，其董事也同时在营利实体的董事会中任职。该基金会的安全与保障委员会有权叫停新模型的发布。
...
OpenAI 旗下的非营利实体（OpenAI Nonprofit）拥有公司价值 1300 亿美元的股份，这使其成为美国最富有的基金会。作为对比，盖茨基金会（Gates Foundation）的资产规模约为 860 亿美元。该非营利实体已承诺投入首批 250 亿美元资金，用于改善全球医疗保健以及加强人工智能安全护栏（AI guardrails）。
...
根据彭博社报道，微软将拥有使用 OpenAI 模型的权利直至 2032 年，该权利涵盖了在双方认定 OpenAI 已开发出 AGI（通用人工智能）之后所构建的模型。微软将继续获得 OpenAI 20% 的收入，并独家拥有其 API 使用权，但其对新云业务不再享有优先购买权。收入分成与 API 协议将持续有效，直至一个独立专家小组验证 OpenAI 已实现通用人工智能。
...
幕后故事：这并非 OpenAI 最初设想的重组。根据一份 2024 年的计划，该公司原本打算废除其非营利架构，转型为一家由风险投资支持的营利性公司。然而，加利福尼亚州和特拉华州的州总检察长对此提案提出了异议。最终达成的妥协方案是：仍由非营利组织掌控大局。
...
OpenAI 需要获得加利福尼亚州和特拉华州的批准，才能确保软银 400 亿美元的投资不至落空，因为这笔投资中有一半是以公司重组及取消投资者回报上限为条件的。同时，OpenAI 还必须征得微软的同意。这使得微软在相关条款的谈判中掌握了重要的筹码。
...
在与加利福尼亚州总检察长的谈判中，OpenAI 承诺将继续留在加利福尼亚州，从而持续接受该州的监管。
...
关键所在：尽管身为非营利组织，OpenAI 的用户基础和估值依然实现了惊人的增长。此次重组给公司带来了必须尽快实现盈利的压力。据报告，该公司的年化收入已超过 130 亿美元，但鉴于其计划在计算基础设施上投入约 1 万亿美元的巨额资金，要实现其宏伟目标，仍需寻求更多融资。
...
不妨这样看：微软对 OpenAI 的早期投资，如今已获得了超值回报。2019 年，当微软首席执行官萨提亚·纳德拉提议公司进行首笔 10 亿美元投资时，比尔·盖茨曾警告道：「这 10 亿美元会打水漂的。」然而，微软累计投入的 130 亿美元，目前估值已高达 1350 亿美元。
...
来自上海 MiniMax 公司的一款开源权重模型，在编码与 AI 智能体（AI Agent）任务的关键基准测试中，向顶尖的专有模型发起了挑战。
...
最新动态：专注于语音聊天和图像生成服务的 MiniMax 公司，开源了其大语言模型 MiniMax-M2 的权重。该模型专门针对代码生成和 AI 智能体（AI Agent）任务进行了优化。

* Input/output: Text in (up to 204,000 tokens), text out (up to 131,000 tokens, roughly 100 tokens per second)

输入 / 输出：文本输入（最多 204,000 个 Token），文本输出（最多 131,000 个 Token，大约每秒 100 个 Token)

* Architecture: Mixture-of-experts transformer, 230 billion parameters total, 10 billion parameters active per token

架构：混合专家（Mixture-of-experts）Transformer，参数总量为 2300 亿，每个 Token 会激活 100 亿个参数。

* Performance: First among open weights models on Artificial Analysis' Intelligence Index

性能：在 Artificial Analysis 的 Intelligence Index（人工智能指数）评测中，于开源权重模型中位列榜首。
...
可用性：模型权重可在 Hugging Face 和 ModelScope 平台免费下载，遵循 MIT 许可证，允许商业及非商业用途；通过 MiniMax 提供的 API 服务，价格为每百万输入 / 输出 Token 0.30 美元 / 1.20 美元。
...
运作方式：MiniMax 尚未发布关于 MiniMax-M2 的技术报告，因此外界对其模型的构建方式知之甚少。
...
给定一个提示词，MiniMax-M2 会在其输出中交错生成推理步骤（用 `<think>...</think>` 标签包裹）。这不同于 DeepSeek-R1 等模型的做法，后者会先生成一整段推理步骤，再给出最终答案。它也不同于 OpenAI GPT-5 及近期的 Anthropic Claude 模型，这些模型同样会在最终输出前生成推理步骤，但通常会将其隐藏或进行总结。
...
MiniMax 建议用户在对话记录中保留 `<think>...</think>` 标签，以确保模型在多轮对话中的最佳表现。因为如果删除这些标签（比如为了节省 Token 数量），将会损害模型的上下文理解能力。
...
结果：MiniMax-M2 在独立评估机构 Artificial Analysis 的 Intelligence Index（一项基于数学、科学、推理和编码基准测试表现的加权平均指标）上获得了 61 分，创下了开放权重模型（open weights models）的新纪录，领先于 DeepSeek-V3.2（57 分）和 Kimi K2（50 分）。其分数落后于专有（闭源）模型：启用了思维功能的 GPT-5（69 分）和 Claude Sonnet 4.5（63 分）。此外，该模型在编码和 AI 智能体任务上表现优异，但同时也显得尤为冗长。为完成 Artificial Analysis 的全部评估，它消耗了 1.2 亿个 Token（文本处理单位），其消耗量与 Grok 4 并列最高。
...
在智能体工具调用测试 τ2-Bench 中，MiniMax-M2（77.2%）的得分超过了 GLM-4.6（75.9%）和 Kimi K2（70.3%），但低于 Claude Sonnet 4.5（84.7%）以及具备思考模式的 GPT-5（80.1%）。
...
在指令遵循基准测试 IFBench 上，MiniMax-M2（72%）的表现显著优于 Claude Sonnet 4.5（57%），但以微弱差距落后于开启了思考模式的 GPT-5（73%）。
...
在 SWE-bench Verified 基准测试（该测试评估需要多文件编辑和测试验证的软件工程任务）中，MiniMax-M2（69.4%）的表现处于中间梯队，领先于 Gemini 2.5 Pro（63.8%）和 DeepSeek-V3.2（67.8%），但落后于 Claude Sonnet 4.5（77.2%）以及启用了思维链（thinking）功能的 GPT-5（74.9%）。
...
在针对命令行任务执行的基准测试 Terminal-Bench 上，MiniMax-M2 以 46.3% 的得分位列第二，仅次于排名第一的 Claude Sonnet 4.5（50%），并且显著领先于 Kimi K2（44.5%）、具备「思考」功能的 GPT-5（43.8%）以及 DeepSeek-V3.2（37.7%）。
...
新闻背后：今年六月，MiniMax 开源了其推理模型 MiniMax-M1 的权重。该模型专为支持长上下文（高达 100 万个 Token）下的 AI 智能体工作流而设计。此前，该公司一直在内部开发用于编码、处理用户反馈及筛选简历等任务的 AI 智能体。但他们发现，主流的闭源模型不仅成本高昂、响应缓慢，而开源模型的能力又有所欠缺。因此，MiniMax 表示，他们打造 MiniMax-M2 正是为了弥补这一市场缺口。
...
其重要性在于：要开发出可靠的 AI 智能体（AI Agent）应用，开发者需要对提示词、工具以及任务分解方式进行各种组合与排列的尝试，而这个过程会消耗海量的 Token。因此，像 MiniMax-M2 这类既能处理智能体任务又具备高成本效益的模型，就能助力更多小型团队借助 AI 智能体实现创新。
...
我们认为：MiniMax-M2 的推理过程可见，这使其决策比那些隐藏或压缩推理步骤的模型更易于审查。随着 AI 智能体（AI Agent）日益被应用于关键任务型场景，推理过程的透明度或许与单纯的性能指标一样重要。
...
音乐生成服务 Udio 宣布，将与全球最大的唱片公司合作，共同打造一个 AI 驱动的流媒体平台。
...
新进展：Udio 计划推出一个付费平台，允许粉丝根据环球音乐集团（UMG）及其旗下厂牌艺人的录音来生成音乐。UMG 旗下艺人包括泰勒·斯威夫特、奥利维亚·罗德里戈、肯德里克·拉马尔等众多畅销歌手。此举是双方达成和解协议的一部分，旨在解决 UMG 于去年提起的诉讼。UMG 在诉讼中指控 Udio 在训练其 AI 音乐模型时侵犯了版权。该和解协议的财务条款、期限及其他具体内容均未对外公开。据音乐行业媒体《Billboard》报道，Udio 仍可自由地与其他音乐厂牌达成类似的合作安排。
...
工作原理：该平台将允许付费用户对现有的录音进行混音、个性化定制和重新组合，并与其他订阅者分享这些创作。
...
艺术家必须事先许可，其作品才能在平台上被使用。同时，艺术家将保留对作品使用方式的控制权，具体可以包括：模仿其声音或音乐风格、将作品从一种风格转换为另一种风格、将不同艺术家的特色融合在一起等诸如此类的操作。
...
艺术家将因其音乐被用于训练 Udio 模型而获得一笔报酬，此外，若其录音被用于生成音乐，还将获得额外的收益。
...
新平台将不允许用户下载由 AI 生成的音乐，也无法通过其他流媒体服务进行分发。作为协议的一部分，Udio 暂时从其现有服务中移除了音乐下载功能，并为订阅用户提供了额外的音乐生成积分，以弥补这一功能缺失。在用户提出投诉后，Udio 暂时恢复了用户对已生成音乐的下载权限。该公司表示，现有服务将继续运营，但其功能会有所调整，不同之处在于将引入音频指纹识别等技术措施。

Other deals: In addition to Udio, UMG forged relationships with other AI music companies that supply tools and technology.

其他合作：除了与 Udio 的合作，环球音乐集团（UMG）还与其他几家提供工具和技术的 AI 音乐公司达成了协议。
...
UMG 和索尼音乐宣布，他们将采用 SoundPatrol 公司开发的音频指纹技术。这项技术的工作原理是比对音频的「特征向量」（learned embeddings），从而识别出那些基于特定原始作品生成的 AI 音乐内容。
...
Stable Audio 2.5 音乐生成器的制造商 Stability AI 宣布，已与环球音乐集团（UMG）达成合作，共同开发专业的音乐制作工具。
...
新闻背后：与图书出版商和电影制片厂类似，唱片公司也已积极采取行动，旨在阻止 AI 公司使用其拥有版权的材料训练模型，并生成可能与其自身作品形成竞争的音乐内容。
...
STIM，一个代表作曲家和录音艺术家收取版税的瑞典组织，制定了一项许可协议，以补偿音乐家因其作品被用于训练 AI 模型（AI models）的行为。
...
去年，索尼音乐（Sony Music）、环球音乐（UMG）、华纳音乐（Warner Music）以及行业组织美国唱片业协会（RIAA）起诉了 AI 公司 Suno 和 Udio，指控其音乐生成 AI 存在版权侵权行为。这些音乐公司分别提起了诉讼，指控被告方使用受版权保护的录音训练其 AI 模型，并在此过程中制作了未经授权的副本，旨在与原告方自身的音乐进行商业竞争。
...
2023 年，环球音乐集团（UMG）向 Apple Music、Spotify 和 YouTube 施压，要求它们采取措施应对 AI 对其艺人的模仿。具体举措包括：阻止 AI 开发者下载其艺人的录音作品，并要求这些流媒体平台不要分发 AI 生成的音乐。
...
为何重要：音乐厂牌与其他媒体公司一样，正面临生成式 AI（Generative AI）对其业务的威胁。这种 AI 能以更低的成本、更短的时间，合成出与它们作品表面相似的产品。法国流媒体音乐服务 Deezer 的一项研究显示，其平台提供的音乐中，有近 28% 是 AI 生成的。今年六月，一个名为 Velvet Sundown 的音乐团体，凭借由 Suno 生成的音乐在 Spotify 上获得了 100 万次播放量。Udio 与 UMG（Universal Music Group）达成的和解，将传统音乐与 AI 生成音乐纳入了同一个商业框架。这表明媒体公司与 AI 公司之间或许能找到共同利益，尽管这也带来了一些副作用，比如限制了 Udio 对其生成音乐的发行。
...
我们认为：由索尼音乐、华纳音乐和美国唱片业协会（RIAA）对 Suno 和 Udio 提起的诉讼仍在审理中。虽然此次与 Anthropic 的协议为解决这些纠纷提供了一个范本，但最终结果如何还远未可知。作为音乐爱好者，我们当然希望能聆听到更多精彩的音乐作品。
...
大语言模型往往会记住其训练数据中的细节，包括那些可能只出现一次的隐私信息，例如个人的姓名、地址或电话号码。研究人员构建了首个保证不会记住这类信息的开放权重语言模型（open-weights language model）。
...
最新动态：来自 Google 的 Amer Sinha、Thomas Mesnard 及其同事发布了 VaultGemma 模型。这是一个拥有 10 亿参数的模型，它采用被称为差分隐私（Differential Privacy）的技术从头开始训练。这种方法能防止模型记住训练数据中那些只出现过一次的样本，其代价是模型性能会有小幅度的下降。该模型的权重文件可免费下载，其附带的许可证允许非商业和商业用途，但附带一些限制条件。
...
差分隐私基础：一个算法（例如训练神经网络）要满足差分隐私，其核心在于：我们无法区分算法在完整数据集上的输出结果（即学习到的模型权重）与在同一数据集**移除任意一个数据样本**后的输出结果有何不同。因为单个样本的存在与否，都不会对最终结果产生显著影响，所以个人信息既不会从结果本身（即模型的权重）泄露，也不会从结果的应用（即模型的输出）中泄露。在神经网络训练中，实现这一目标的方法之一是限制每个样本的梯度对模型权重更新的影响。例如，可以向每个样本的梯度添加噪声，从而使得该样本的梯度特征更难与其他样本的梯度区分开来。
...
关键洞察：以往的大多数研究在微调模型时才应用差分隐私（Differential Privacy），但这无法防止模型在预训练阶段就记住某些训练样本。一旦隐私信息被编码进模型的权重中，后续的微调过程就无法可靠地将其移除。而从训练伊始就采用差分隐私，则能确保这类敏感信息不会固化到模型内部。
...
工作原理：VaultGemma 采用了与 Google Gemma 2 的 10 亿参数版本相同的 Transformer 架构。不仅如此，研究团队还使用与 Gemma 2 完全相同的数据集 —— 一个包含网络文本、代码和科学文献的 13 万亿 Token 语料库 —— 从头开始对 VaultGemma 进行了预训练。在模型学习预测长达 1024 个 Token 的序列中下一个 Token 的过程中，研究人员应用了差分隐私技术。
...
对于批次中的每一个样本，作者计算其梯度并进行裁剪，以确保单个样本对权重更新的贡献被限制在一个固定的阈值之内。这样就能保证，任何一个样本都不会相对于其他样本，对权重更新产生过大的影响。
...
作者计算了每个批次中经过裁剪（clipped）的梯度的平均值，并在更新模型权重前，向这个平均值添加了高斯噪声。这种噪声削弱了数据集中独特样本的影响，同时让那些重复出现的样本所贡献的梯度信号得以凸显。最终，这使得训练出的模型权重，在统计学上与一个未使用任何特定 1,024-token 序列进行训练的模型的权重无法区分。
...
结果：VaultGemma 在从训练集中随机采样的一百万个序列上，未检测到可测量的记忆（memorization）现象；而参数量大致相当的预训练 Gemma 1、2 和 3 模型则表现出了这种记忆。VaultGemma 在 7 项问答基准测试上的平均性能，大体与 GPT-2（15 亿参数）持平，但不及参数量相近的 Gemma 模型。
...
为了衡量 Gemma 系列模型的记忆（Memorization）效应，研究人员统计了模型在只看到序列前半部分时，能完整复现的训练数据所占的百分比。测试结果显示，拥有 10 亿参数的 Gemma 3 复现了 0.0005% 的数据；参数量为 20 亿的 Gemma 2 复现了 0.04%；同样是 20 亿参数的 Gemma 1 复现比例最高，约为 1%；而 VaultGemma 的复现比例则为 0%。
...
VaultGemma 在 HellaSwag 基准测试中取得了 39% 的准确率，该测试用于评估模型在日常场景中的常识推理能力。GPT-2 的准确率为 48%，而拥有 10 亿参数的 Gemma 3 模型则达到了 61% 的准确率。
...
在用于测试事实性问答能力的 TriviaQA 数据集上，VaultGemma 的准确率为 11%，GPT-2 为 6%，而拥有 10 亿参数的 Gemma 3 模型则达到了 40%。
...
是的，不过这里有个前提：这种隐私保护仅适用于那些独一无二的个例，比如某个在数据集中只出现过一次的私人电话号码。如果私人信息在训练数据中反复出现 —— 例如，某位名人的住址遭泄露，并在多篇公开报道中出现 —— 那么模型就可能将其作为一种通用模式来习得。
...
为什么这很重要：个人隐私信息可能会混入 AI 模型的训练数据集中。在常规使用时，一个典型的大语言模型（LLM）可能会在未经用户同意的情况下泄露这些信息。而 VaultGemma 的研究表明，大型的开放权重模型同样可以实现可验证的隐私保护。当然，这种隐私保护目前仍需付出性能上的代价 —— VaultGemma 1B 的性能水平仅与大约五年前的模型相当。尽管如此，其研究成果依然充满希望，未来的工作有望弥合这一性能差距。

We're thinking: The smartest model for sensitive data might be the one that remembers only the most common information.

我们的观点是：在处理敏感数据时，最明智的模型策略或许是只让它记住最常见的信息。
