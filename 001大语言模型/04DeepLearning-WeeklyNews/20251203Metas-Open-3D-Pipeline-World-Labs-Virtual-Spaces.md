## 20251203Metas-Open-3D-Pipeline-World-Labs-Virtual-Spaces
...
根据爱德曼（Edelman）的调查，在美国，49% 的人对日益广泛的人工智能（AI）应用持排斥态度，而仅有 17% 的人表示欢迎。与之形成鲜明对比的是，在中国，持排斥态度的人占 10%，而欢迎者高达 54%。皮尤（Pew）的数据也显示，许多其他国家对 AI 应用的接纳热情也远高于美国。
...
需要明确的是，我们所有从事人工智能（AI）工作的人都应仔细审视其益处与危害（例如，深度伪造（Deepfakes）技术污染社交媒体，以及存在偏见或不准确的 AI 输出误导用户），如实阐述这两方面，并在努力扩大益处的同时，积极着手解决问题。然而，关于 AI 危险的过度炒作，已经实实在在地损害了我们这个领域的公信力。这类炒作在很大程度上源于一些领先的 AI 公司，它们试图通过将自身技术与核武器相提并论等方式，来彰显其技术的超凡威力。不幸的是，相当一部分公众对此深信不疑，甚至认为 AI 可能引发世界末日。AI 界必须停止这种自损行为，努力重新赢得社会的信任。
...
首先，为了赢得人们的信任，我们仍有许多工作要做，以确保人工智能（AI）能够广泛地造福所有人。在普通大众看来，"更高的生产力」常常是「我的老板会赚更多钱」的委婉说法，甚至更糟，意味着裁员。ChatGPT 固然令人惊叹，但我们仍需努力开发出能对人们生活产生更大积极影响的应用程序。我认为，为人们提供培训将是解开这一困局的关键。DeepLearning.AI 将继续在 AI 培训领域担当先锋，但我们需要的努力远不止于此。
...
其次，我们必须真正做到值得信赖。这意味着我们每个人都必须避免夸大其词或危言耸听，尽管有时可能会为了博取关注，或是为了游说政府通过某些法律来遏制竞争对手（例如开源产品），而产生这样做的冲动。
...
我希望我们的社区也能勇于指出那些传播不实炒作的新闻报道。例如，Nirit Weiss-Blatt 曾撰写一篇精彩文章，揭露了《60 分钟》节目对 Anthropic 一项研究的报道存在严重误导。该研究进行了一次红队演练（red-teaming exercise），即研究人员经过大量坚持不懈的努力，最终将 AI 系统 Claude 逼入特定情境（例如以关闭其作为威胁），才诱使其表现出所谓的「勒索」行为。然而，新闻报道扭曲了这一事实，让许多人误以为这种「勒索」行为是 AI 自发产生的，而非研究人员精心设计诱导的结果。这导致公众对 AI 实际「密谋策划」的频率产生了极度夸大的错误印象。红队演练对于发现系统漏洞至关重要，但《60 分钟》这种被广泛传播的误导性报道，其带来的炒作危害可能会在很长一段时间内损害人工智能领域的健康发展。
...
身处硅谷，我意识到自己生活在一个 AI 爱好者的同温层里，这非常有利于交流想法并鼓励彼此进行创造！与此同时，我也认识到 AI 确实存在诸多问题，需要 AI 社区来解决。我经常与来自各行各业、不同背景的人们交流。我曾与担忧 AI 会使其作品贬值的艺术家交谈，与因就业市场严峻而焦虑、并担心 AI 是否会让处境雪上加霜的大四学生交谈，还与那些担心孩子沉迷于聊天机器人并从中获取有害建议的家长们交谈。
...
我不知该如何解决所有这些问题，但我会竭尽全力去解决我能解决的。我也希望你能如此。唯有我们所有人都共同努力，才能重新赢得社会的信任。
...
其重要性在于：SAM 系列模型提供了一个统一的流程，能够从图像生成 3D 模型。该系列的每一代模型都代表了业界先进水平，不仅能根据文本指令实现更精准的图像分割，还能生成更受人类评估者青睐的 3D 物体和人体模型。这些模型已经在推动 Meta 平台用户体验的创新。例如，借助 SAM 3 和 SAM 3D，Facebook Marketplace 的用户可以预览家具或其他家居装饰摆放在特定空间中的实际效果。
...
背景解析：早期的生成式 AI（Generative AI）模型能够动态生成 3D 空间，但这类空间通常无法保存，也无法再次进行交互访问。Marble 的突出之处在于，它能生成可保存和编辑的空间。例如，World Labs 公司在十月推出了 RTFM，它可以在用户探索空间的同时进行实时生成。像 Decart 和 Odyssey 这样的竞争初创公司已经提供了演示版本，而谷歌的 Genie 3 目前仍是一个研究预览项目。
...
其重要性在于：World Labs 创始人兼斯坦福大学教授李飞飞指出，空间智能 —— 即理解物理物体如何占据空间并在此间运动 —— 是智能的一个关键方面，而当前的语言模型难以充分应对。World Labs 希望通过 Marble 项目，能够像 ChatGPT 及其后续的大语言模型推动文本处理领域的飞跃那样，催化空间人工智能（Spatial AI）的发展。
...
百度发布了两款模型：一款是轻量级的、开源权重的视觉语言模型，另一款则是庞大的专有多模态模型，旨在与美国竞争对手一较高下。
...
Ernie-4.5-VL-28B-A3B-Thinking：百度的这款新开放权重模型，是在其早期的纯文本混合专家（MoE）推理模型 Ernie-4.5-21B-A3B Thinking 基础上，增加了一个 70 亿参数的视觉编码器以处理图像。在视觉推理任务上，其性能超越了同类乃至规模更大的模型。该模型能够提取屏幕文本、对视频进行跨时间维度分析，并能调用工具来放大图像细节或搜索相关图像。

* Input/output: Text, image, video in (up to 128,000 tokens); text out

* 输入 / 输出：文本、图像、视频（最多 128,000 个 Token）；文本输出
...
* 架构：混合专家模型（Mixture-of-experts，MoE）Transformer（总参数量为 280 亿，每个 Token 会激活其中 30 亿个参数），包含一个 210 亿参数的语言解码器 / 编码器。
...
*  **训练**：作者在一种称为中期训练（mid-training）的新兴阶段使用了视觉 - 语言推理示例。这个阶段通常利用中等规模的数据集，旨在进行微调之前，专门提升模型的特定技能或让其熟悉特定领域知识。此外，他们还通过多模态数据的强化学习（RL）来微调模型。由于混合专家（MoE）架构在强化学习过程中容易失稳，研究团队结合使用了 GSPO 和 IcePop 这两种方法来确保微调的稳定性。

* Features: Tool use, reasoning

* 特性：工具使用（Tool use）、推理（Reasoning)
...
* 性能：百度表示，尽管仅动用了 30 亿个参数，文心大模型 4.5 VL-28B-A3B-Thinking（Ernie-4.5-VL-28B-A3B-Thinking）在文档理解任务上的表现足以媲美规模更大的专有模型。例如，在 ChartQA（图表解读）基准测试中，其准确率达到了 87.1%，超过了 Gemini 2.5 Pro（76.3%）和设置为高推理模式的 GPT-5（78.2%）。在 OCRBench（图像文本识别）基准测试中，它取得了 858 分，领先于设置为高推理模式的 GPT-5（810 分），但略低于 Gemini 2.5 Pro（866 分）。
...
*  可用性：模型权重可通过 HuggingFace 获取，在 Apache 2.0 许可证下可免费用于非商业及商业用途。API 调用服务可通过百度千帆平台获得，费用为每百万输入 / 输出 Token（Token）0.14/0.56 美元。

* Undisclosed: Output size limit, training data, reward models

*  **未公开信息**：模型的输出长度限制、训练数据细节以及奖励模型（Reward Model）的具体构成。
...
Ernie-5.0：百度将 Ernie-5.0 的方法描述为原生多模态（natively multimodal），这意味着它是在文本、图像、音频和视频数据上进行联合训练的，而非在训练完成后才融合不同的媒体编码器，或是将输入分配给专门的模型进行处理。百度表示，其性能与同属多模态模型的 Google Gemini 2.5 或 OpenAI GPT-5 相当。

* Input/output: Text, image, audio, and video in (up to 128,000 tokens); text, image, audio, video out (up to 64,000 tokens)

* 输入 / 输出：支持文本、图像、音频和视频输入（上限为 128,000 Token）；输出文本、图像、音频和视频（上限为 64,000 Token）。

* Architecture: Mixture-of-experts (MoE) transformer (2.4 trillion parameters total, less than 72 billion active per token)

* 架构：专家混合（Mixture-of-experts，MoE）Transformer（总参数 2.4 万亿，每个 Token 激活的参数不到 720 亿)

* Features: Vision-language-audio understanding, reasoning, agentic planning, tool use

* 功能：具备视觉、语言与音频的多模态理解与推理能力，支持 AI 智能体（AI Agent）任务规划与工具调用。
...
*  性能：根据百度进行的多模态推理、文档理解以及视觉问答测试，其报告显示，文心大模型 5.0（Ernie-5.0）的表现与 OpenAI 的 GPT-5（设置为高推理模式）以及 Google 的 Gemini 2.5 Pro 相当甚至更优。具体来说，在文档理解任务 OCRBench 和 DocVQA，以及结构化数据推理任务 ChartQA 上，文心 5.0 都取得了顶尖的成绩。此外，在多模态音频理解数据集 MM-AU 和声学场景分类数据集 TUT2017 上，百度称其模型也表现出了强大的竞争力，但未公布具体的分数指标。

* Availability: Free web interface, API $0.85/$3.40 per million input/output tokens via Baidu Qianfan

*  可用性：提供免费的网页界面。通过百度千帆平台调用 API 的费用为，每百万输入 Token 0.85 美元，每百万输出 Token 3.40 美元。

* Undisclosed: Training data, training methods

* 未披露：训练数据（Training data），训练方法（Training methods)
...
不过，在文心一言 5.0（Ernie-5.0）发布后不久，就有开发者报告称，该模型在被明确要求停止后，仍然会反复调用工具。百度已承认存在此问题，并表示正在修复中。
...
为何重要：Ernie-4.5-VL-28B-A3B-Thinking 以极低的成本（仅为竞争模型的一小部分）就具备了顶尖的视觉推理能力，并且在模型微调和其他商业定制方面也更为灵活。然而，备受期待的 Ernie 5.0 的表现似乎有些令人失望。它在部分视觉任务上可以与顶级模型媲美，但在 LM Arena 等权威排行榜上，并未能跻身最前沿模型（如 Qwen3-Max 和 Kimi-K2-Thinking）的行列。对文本、图像、视频和音频进行统一预训练，是一种相对较新的技术路径。这种方法有望简化当前那些需要为不同媒体类型分别组合编码器和解码器的复杂系统。
...
在工厂中，多台机械臂常在紧凑的空间内协同作业。传统上，为了避免它们相互碰撞，需要手动为其规划运动轨迹。如今，研究人员利用通过强化学习训练的图神经网络，实现了这一编程过程的自动化。
...
新进展：来自谷歌 DeepMind、伦敦大学学院以及机器人软件公司 Intrinsic 的研究人员马修·赖、基根·高及其同事，开发了一套名为 RoboBallet 的图神经网络，用于协调多个机械臂的动作。
...
关键洞察：对于传统的基于搜索的规划器而言，协调多台机器人手臂在计算上难以实现。这类规划器的工作原理，是通过搜索机器人关节所有可能的运动轨迹，并同时检测是否会发生碰撞，从而规划出一条抵达目标的路径。每增加一台机器人或一个障碍物，可能路径的配置数量就会呈指数级增长。图神经网络（Graph Neural Network）能够克服这一瓶颈。它通过在大量模拟场景中进行学习，这些场景包含了不同的机器人排布、障碍物和目标位置，从而能够生成同步且无碰撞的运动方案。
...
工作原理：RoboBallet 是一个图神经网络。其输入是机器人、障碍物和目标的位置与方向（姿态），输出则是每个机械臂从当前位置运动到目标位置所需的关节速度。作者完全在仿真环境中，使用一种名为 TD3 的演员 - 评论家（actor-critic）强化学习算法对模型进行了训练。他们生成了大约 100 万个仿真工作空间。每个工作空间包含一个由 4 或 8 个模拟的 3 关节 Franka Panda 机械臂组成的团队，这些机械臂被随机安置在桌子的各侧，同时空间中还随机放置了 30 个障碍物块，并为每个团队设定了 40 个目标位置和方向。对于那些在初始状态就发生碰撞的场景，他们予以了排除。
...
* 给定一个工作空间，作者将机器人、障碍物和目标位置都表示为图中的节点。图中的边则连接着每个机器人的末端执行器（tip）与其目标位置、每一个障碍物，以及其他每一个机器人。这些边的嵌入向量（edge embeddings）编码了机器人末端执行器相对于某个物体或目标位置的中心点（centroid）、尺寸和方向的具体关系。
...
* 在训练过程中，模型每 100 毫秒为所有机器人选择一次关节速度，从而指挥每只机械臂如何运动（这对应演员 - 评论家算法中的「演员」角色）。与此同时，模型会评估每个预测的好坏 —— 即，当前动作及其后续可能动作预计能获得多少总奖励（这对应算法中的「评论家」角色）。
...
* 作者对模型进行了奖励与惩罚：当机械臂触及目标位置时给予奖励，发生碰撞时则施加惩罚。由于机械臂很少能成功触及目标位置，他们采用了后见之明经验回放（Hindsight Experience Replay）。这种方法能将失败的尝试转化为有用的学习样本，其原理是将机械臂偶然到达的位置，重新设定为它本应追求的目标。
...
* 这一损失函数鼓励智能体（actor）采取那些被评判器（critic）预测能获得更高长期回报的动作。这有助于模型学会更倾向于选择从长远看有收益的行为，而不是一味追求即时奖励。
...
结果：研究团队在真实物理环境中对训练完成的模型进行了测试。在此次实测中，他们基于一个物理工作空间的已知几何结构，结合机器人的预设位置和障碍物的三维网格，生成了相应的图。

* Given new work spaces, the model generated collision-free trajectories for up to 8 Franka Panda robotic arms.

* 针对新的工作空间，该模型为多达 8 个 Franka Panda 机械臂生成了无碰撞的运动轨迹。
...
* RoboBallet 有效地实现了工作并行化。机器人移动到 20 个目标位置所需的平均时间，从使用 4 条机械臂时的 7.5 秒缩短至使用 8 条机械臂时的 4.3 秒。
...
* 在一个简化的基准测试中（包含四个机器人和 20 个目标位置），RoboBallet 生成轨迹的速度与最佳的手动优化基线相当，均能在 8 到 11 秒的时间范围内抵达所有目标位姿。
...
为何重要：RoboBallet 项目展示了，一种学习算法能够协调多个机器人在实际场景中协同工作，并且该算法仅需在模拟环境中进行训练即可实现。此外，这种模型也更加鲁棒。当某个机器人发生故障时，预设的固定程序无法做出调整。相比之下，图神经网络（Graph Neural Network）能够持续追踪机器人、任务与障碍物之间的动态关系。一旦有机器人失效，它便可以实时适应，并调整后续的行动计划。
...
我们的思路是：将世界表征为图（graph），相当于为数据赋予了一种内在的结构，能够记录物体间的相对位置和关系。而其他数据结构本身并不具备物体间的关联信息，因此，基于它们进行学习的网络模型，就必须额外学习这些关系。使用图结构能让网络更轻松地学会如何完成任务，因为它无需再从头学习这些关系。
