## 20251126Google-Rules-Arena-Leaderboards-Microsoft-Anthropic

[Google Rules Arena Leaderboards, Microsoft+Anthropic, Record Labels Back AI Music, and more...](https://www.deeplearning.ai/the-batch/issue-329/)
...
人工智能（AI）领域存在泡沫吗？随着巨额资金涌入 AI 基础设施 —— 例如 OpenAI 提出的 1.4 万亿美元计划，以及英伟达（Nvidia）市值一度突破 5 万亿美元 —— 许多人不禁质疑，投机与炒作是否已将 AI 投资的价值推高到了不可持续的水平。然而，人工智能并非铁板一块，其不同细分领域的泡沫化程度也各不相同。

1 AI application layer: There is underinvestment. The potential is still much greater than most realize.

AI 应用层：投资仍然不足。其潜力远比大多数人想象的要大。

2 AI infrastructure for inference: This still needs significant investment.

AI 推理基础设施：仍需大量投资。

3 AI infrastructure for model training: I'm still cautiously optimistic about this sector, but there could also be a bubble.

用于模型训练的 AI 基础设施：我对这个领域仍然保持谨慎乐观，但也可能存在泡沫。
...
AI 应用层。未来十年，人们将利用新的人工智能技术构建出许多前所未有的应用。从定义上几乎可以说，构建在 AI 基础设施或技术（例如大语言模型（LLM）API）之上的应用，其价值必须高于底层基础设施。因为正是这些应用产生的价值，才足以支撑并向基础设施与技术的提供方支付费用。
...
我看到众多企业正在应用智能体工作流（Agentic Workflows），并呈现出蓬勃发展的积极信号，我坚信这一趋势将会持续壮大！我也与许多风险投资（Venture Capital）者交流过，他们对于投资 AI 应用（AI Applications）犹豫不决，原因是自认缺乏甄别优胜者的能力；相比之下，如何部署 10 亿美元来构建 AI 基础设施，其路径则更为清晰明了。其中一些人也受到了市场炒作的影响，认为几乎所有 AI 应用都将会因为前沿大语言模型（LLM）公司改进其基础模型（Foundation Models）而被轻易取代。总体而言，我认为 AI 应用领域存在着巨大的投资缺口。这仍然是我的风险工作室 AI Fund 重点关注的核心方向。
...
AI 推理的基础设施。尽管人工智能（AI）目前的渗透率还很低，但基础设施提供商已经难以满足生成 Token 所需的计算能力需求。我手下的好几个团队都在担心，我们能否获得足够的推理算力；同时，成本和推理吞吐量也限制了我们使用更多的算力。企业面临供应受限而非需求受限，这其实算是一个「好」问题。相比之下，需求受限（即没有足够的人想要你的产品）是一个更为常见的问题。但供应不足毕竟也是个问题，因此，我很高兴看到我们整个行业正在大力投资，以扩展推理能力。
...
作为 Token 生成需求旺盛的一个具体例证，具备高度自主性的编码 AI 智能体正在飞速进步。我一直是 Claude Code 的爱好者；随着 GPT-5 的发布，OpenAI Codex 也取得了巨大改进；Gemini 3 则让 Google CLI 变得极具竞争力。随着这些工具的不断完善，它们的采用率将会上升。然而，目前整体市场渗透率仍然很低，许多开发者还在使用旧一代的编码工具（有些甚至完全没有使用任何智能编码工具）。鉴于这些工具非常实用，我确信市场渗透率将会提高 —— 而这将推动 Token 生成的总体需求不断增长。
...
我在去年年初就预测过，我们将需要更多的推理算力，部分原因在于 AI 智能体（AI Agent）工作流的兴起。自那时起，这一需求变得尤为突出。整个社会都需要提升 AI 推理的算力！
...
不过，我并不是说投资这个领域就稳赚不赔。如果我们最终出现了过度建设 —— 虽然我目前无法断定是否会发生 —— 那么服务提供商可能将不得不亏本或以极低的利润出售其（计算）能力。我当然希望这个领域的投资者都能赚钱。但好消息是，即便真的过度建设了，这些过剩的产能最终也会被市场消化掉，而这对于应用开发者来说反而是件好事！
...
用于模型训练的 AI 基础设施。看到越来越多的资金涌入训练更大规模模型的领域，我深感欣喜。然而，在（前述的）这几类投资方向中，这似乎是风险最高的。如果开源 / 开放权重的模型持续扩大其市场份额，那么那些斥资数十亿美元用于训练专有模型的公司，其投资回报率可能将难以达到预期。
...
此外，得益于算法和硬件的不断改进，每年训练一个具备同等能力水平的模型，其成本都在下降。因此，训练最前沿模型的「技术护城河」正在变得薄弱。（不过，ChatGPT 已经建立起强大的消费级品牌，因而拥有坚实的品牌护城河；而 Gemini 凭借谷歌庞大的分销网络优势，也展现出强劲的势头。）
...
我总体上仍然对人工智能（AI）投资持乐观态度。但下行风险是什么？换言之，是否存在一个即将破裂的泡沫？有一个情景令我担忧：如果人工智能技术栈的某个部分（例如训练基础设施）因过度投资而崩溃，可能会引发更广泛的 AI 负面市场情绪，导致资金非理性地从 AI 投资领域撤出，尽管该领域整体基本面依然强劲。我认为这不会发生，但万一发生，那将非常可惜，因为在我看来，AI 领域仍有大量工作极其值得获得更多投资。
...
沃伦·巴菲特曾广为传播本杰明·格雷厄姆的一句名言：「短期来看，市场是一台投票机；但长期来看，它是一台称重机。」他想说的是，股票价格在短期内由投资者的情绪和投机行为主导；而从长期看，则由基本面和内在价值决定。我自认难以预测市场情绪和投机波动，但对于人工智能（AI）基本面（fundamentals）的长期前景，我抱有极强的信心。因此，我的计划很简单：继续脚踏实地，坚持做下去！
...
Google 推出了其旗舰视觉语言与图像生成模型 Gemini 3 Pro 和 Gemini Nano，并将它们部署给全球数十亿用户。

Gemini 3 Pro：这是一款多模态推理模型。截至本文撰写时，它在 LMArena 的文本、WebDev 和视觉排行榜上均处于领先地位。本次更新的一项关键变化是，它用推理级别设置（低、中、高）取代了 Gemini 2.5 中为推理任务分配的固定 Token 预算。Google 表示，这种新方式更易于管理。
...
* 输入 / 输出：支持文本、图像、PDF、音频和视频输入（最多 100 万个 Token），输出为文本（最多 64,000 个 Token，每秒 128 个 Token）。
* 架构：混合专家（Mixture-of-experts）Transformer 架构
...
* **训练**：首先在从网络抓取的数据（文本、代码、图像、视频和音频）、授权数据、Google 用户数据以及合成数据上进行了预训练（Pre-training）。随后，通过微调（Fine-tuning）使其学会推理、遵循指令，并与人类偏好保持一致；微调使用的是代表多步推理、问题解决和定理证明的数据，并采用了未具体说明的强化学习方法。
...
* 功能：工具调用（Google 搜索、读取网页 URL 内容、执行 Python 代码、文件搜索、函数调用），结构化输出，可调节的推理强度（低、中、高)
...
*  性能：在谷歌的测试中，Gemini 3 Pro 在多个基准测试中刷新了最高水平（state of the art），包括 Humanity's Last Exam（推理）、GPQA Diamond（学术知识）、AIME 2025（竞赛数学问题）、MMMU-Pro（多模态推理）和 MRCR v2（长上下文性能），部分项目的领先优势相当显著。在大约一周的时间里 —— 直到 Anthropic 的 Claude Opus 4.5 后来居上之前 —— 它还在 SWE-bench Verified（智能体编码）、Terminal-Bench 2.0（智能体终端编码）和 ARC-AGI-2（视觉推理谜题）这几个榜单上位居首位。
...
*  **可用性**：
  *  **免费版本**：可通过 Gemini 应用以及 Google 搜索中的 AI 概览（AI Overviews）功能免费使用。
  *  **付费集成**：已集成到付费服务中，包括 Google AI Studio、Vertex AI 以及 Google Antigravity 智能体编码工具（agentic coding tool）。
  *  **API 价格**：
    *  **标准上下文长度（小于 20 万 Token)**：输入 Token 每百万个 2 美元，缓存 Token 每百万个 0.20 美元，输出 Token 每百万个 12 美元。
    *  **长上下文长度（大于 20 万 Token)**：输入 Token 每百万个 4 美元，缓存 Token 每百万个 0.40 美元，输出 Token 每百万个 18 美元。此外，对于长上下文模型，缓存 Token 还需额外支付每小时每百万个 4.50 美元的费用。
...
是的，不过：Gemini 3 Pro 消耗了大量的 Token 才实现了其出色的性能。完成「人工分析智能指数」（Artificial Analysis Intelligence Index，一个涵盖 10 项基准测试的加权平均值）的成本高达 1,201 美元，仅次于 Grok 4（1,888 美元）。此外，在它本应选择「不予回答」的情况下，它却给出了错误输出。根据「人工分析全知幻觉率」（Artificial Analysis Omniscience Hallucination Rate）的测试结果 —— 该指标计算的是所有未能正确回答的尝试（包括模型拒绝回答的情况）中，错误答案所占的比例 ——Gemini 3 Pro 的幻觉率高达 88%，远高于 Claude Sonnet 4.5（48%）和 GPT 5.1 High（5%）。
...
Nano Banana Pro：谷歌还发布了 Nano Banana Pro（亦称 Gemini 3 Pro Image），目前它在 Artificial Analysis 的**文生图（Text-to-Image)**和**图像编辑（Image Editing)**排行榜上均位居榜首。Nano Banana Pro 在生成和编辑图像时，会运用 Gemini 3 Pro 的推理与知识能力，首先生成最多两张中间图像，用以推敲构图和逻辑，然后再输出最终图像。该模型专精于文本生成，并能在多次生成过程中保持最多 5 个角色形象的一致性。此外，它还能借助谷歌搜索来确保图像内容的事实准确性，从而生成准确的信息图、地图等，并且可以在保持原有艺术风格的前提下，对图像中的文字进行翻译或修改。
...
* 输入 / 输出：输入为文本或图像（Token 上限 100 万个，参考图像上限 14 张），输出为图像（Token 上限 64,000 个；分辨率支持 1024x1024、2048x2048 或 4096x4096 像素)

* Architecture: Based on Google Gemini 3 Pro

* Training: Same as Google Gemini 3 Pro

*  架构：基于谷歌 Gemini 3 Pro 模型
*  训练：与谷歌 Gemini 3 Pro 的训练方式相同
...
* 功能：输出内容带有 SynthID 生成的水印；具备默认的优化处理功能，可在最终输出前自动精炼图像构图；与 Google 搜索以及 Adobe、Figma 等创意工具集成；支持对图像中的多个图形元素、文字以及涂鸦（即用户随手在图片上画的草图）进行编辑。
...
*  **性能表现**：根据谷歌进行的人工评测，Nano Banana Pro 在所有测试任务中的得分均领先于 OpenAI GPT-Image 1、Gemini 2.5 Flash Image、ByteDance Seedream v4 以及 Black Forest Labs Flux Pro Kontext Max。在文本渲染测试中，Nano Banana Pro（Elo 分数 1,198）的表现优于排名第二的 GPT-Image 1（Elo 分数 1,150）。在信息图生成任务中，Nano Banana Pro（Elo 分数 1,268）也击败了 Gemini 2.5 Flash Image（Elo 分数 1,162）。
...
*  **可用性与定价**：
  *  **访问途径**：可通过以下方式使用：
    *  全球范围内的 Gemini 应用，需选择「思考（Thinking）」和「生成图像（Create Images）」功能（使用配额因用户等级而异，包含免费等级）。
    *  Google 搜索中的「AI 模式」（目前仅面向美国地区的 Google AI Pro 和 Ultra 订阅用户开放）。
    *  Google Ads。
    *  Google Workspace（具体在 Slides 和 Vids 应用中）。
    *  NotebookLM。
    *  Gemini API、Google AI Studio、Vertex AI 以及 Google Antigravity。
  *  **API 价格**：
    *  输入图像：每张 0.0011 美元。
    *  输出图像：每张 0.134 美元（分辨率为 1024x1024 或 2048x2048 像素），或 0.24 美元（分辨率为 4096x4096 像素）。
...
* 知识截止时间：2025 年 1 月
* 未披露：参数量（Parameter count）、架构细节、训练方法
...
新闻背后：相较于 Anthropic 八月发布的 Claude Opus 4.1 或 OpenAI 十一月初发布的 GPT-5.1，谷歌此次对 Gemini 3 Pro 和 Nano Banana Pro 的推广范围要广泛得多。谷歌并没有以开放 API（应用程序编程接口）和发布少量新应用打头阵，而是直接将新模型整合到了每月用户超过 20 亿的各项服务中，包括谷歌搜索的 AI 概览（AI Overview）、Gmail、Docs、Sheets 以及 Android 系统。与此同时，谷歌还推出了 Antigravity，这是一个具备 AI 智能体能力的编码平台，旨在与 Cursor 和 Claude Code 等工具竞争。
...
其重要性在于：在多项基准测试中落后于 OpenAI 和 Anthropic 数月后，谷歌（Google）如今已在许多方面取得了领先（尽管一周后发布的 Claude Opus 4.5 带来了一些变数）。对于正在评估该选用哪个模型的开发者而言，这一进展可能改变他们的默认选项。总体来看，进入 2025 年后，基准测试的榜首位置已多次更迭，这表明尚无任何一家公司建立起稳固的技术优势。
...
我们的看法是：尽管 Gemini 3 Pro 在十几个主流基准测试中确立了当前的最先进水平 —— 当然，这至少是本周的情况 —— 但谷歌的市场实力和分销渠道优势或许更为关键。谷歌能够通过其现有的产品体系，瞬间将技术部署给数十亿用户，这构筑了一道宽阔的竞争护城河。对于大多数竞争对手而言，除了拥有 iPhone 生态帝国的苹果，想要仅仅依靠发布更优秀的模型来跨越这道鸿沟，恐怕是相当困难的。
...
在与长期合作伙伴 OpenAI 修订协议后不久，微软承诺向 OpenAI 的主要竞争对手之一 Anthropic 投资数十亿美元。
...
最新消息：微软、Anthropic 和英伟达宣布达成合作伙伴关系。微软和英伟达将分别向 Anthropic 投资 100 亿美元和 50 亿美元。微软将在其云平台上提供 Anthropic 的 AI 模型，而 Anthropic 则承诺在微软的基础设施上购买价值 300 亿美元的 AI 推理计算服务。包括部分投资是否为可选或与 Anthropic 的业绩挂钩在内的其他具体条款，目前尚未披露。
...
具体来说：这项协议使得 Anthropic 的 Claude 模型成为唯一一个能在微软、谷歌和亚马逊这三大领先云服务上全面提供的顶级模型。此举也大幅提升了 Anthropic 的公司估值。
...
* Claude Sonnet 4.5、Claude Haiku 4.5 和 Claude Opus 4.1 已在 Microsoft Foundry 平台推出预览版。微软还将这些大语言模型集成到了 Excel 的智能体（Agent）模式中，从而让 AI 能够自动执行构建、编辑和分析电子表格的任务。
...
* Anthropic 承诺在 Azure 上购买推理算力，并基于其 Nvidia Grace Blackwell 和 Vera Rubin 硬件，以未公开的价格签订了高达 1 吉瓦的额外算力合同。这类似于 Anthropic 在去年 10 月与 Google 签约购买的「数十亿美元」算力。
...
* Nvidia 与 Anthropic 将开展合作，旨在让 Anthropic 的模型适配 Nvidia 硬件，并针对这些模型优化 Nvidia 的 GPU。此前，Claude 主要运行在 Amazon 或 Google 的硬件上。
...
* 据 CNBC 报道，这些投资对 Anthropic 的估值约为 3500 亿美元，较其去年 9 月 1830 亿美元的估值大幅上涨。

新闻背后：微软在 2022 年与 OpenAI 建立的合作伙伴关系，为 Anthropic 在 2023 年与亚马逊结盟铺平了道路，形成了一家 AI 初创公司与一家老牌云服务商的搭配。然而，随着 Anthropic 后来与谷歌达成协议，以及 OpenAI 完成资本重组并调整了与微软的关系，微软和 Anthropic 之间找到共同利益就变得更容易了。
...
* 在十月对微软与 OpenAI 早期协议进行的一次修订中，微软获得了 OpenAI 新成立的营利性子公司 27% 的股份，以及在由专家小组判定 OpenAI 实现通用人工智能（AGI）之前，获得其 20% 的收入。微软有权使用 OpenAI 的模型直至 2032 年，但该权利并非独家；OpenAI 可以在部分业务上与其他云服务提供商合作。
...
* 九月，微软在其 Copilot 编码助手和 Microsoft 365 生产力套件中接入了 Claude 模型。随后，微软允许这些模型访问存储在自家云服务中的文档和电子邮件。
...
* 早在 2023 年秋季，微软就寻求减少对 OpenAI 的依赖，并开发自身的尖端人工智能（AI）能力。一年后，随着 OpenAI 着手重组并与甲骨文（Oracle）签署了一项独立的云服务协议，双方关系变得紧张。与此同时，微软聘请了 Inflection AI 的联合创始人 Mustafa Suleyman，将其 AI 技术整合到消费级产品中。
...
* 2023 年 10 月，Anthropic 与亚马逊达成协议，将独家使用亚马逊的基础设施来训练其模型，协议金额高达 40 亿美元。就在同月，Anthropic 又与谷歌建立了价值 20 亿美元的合作伙伴关系，谷歌由此成为其 AI 模型 Claude 的推理合作伙伴（inference partner）。
...
**重要性何在**：几年前，OpenAI 是颗急需算力的 AI 界新星，而微软则既需要先进技术来与对手竞争，也需要为其 Azure 平台吸引客户。双方建立了合作伙伴关系，微软通过数轮投资共投入 1700 万美元，实现了双赢。然而时至今日，情况已变：OpenAI 所需的算力已超出微软所能或所愿提供的范围，而微软也需要使其 AI 产品线多元化。与此同时，Anthropic 的模型变得大受欢迎，尤其在微软的核心客户群 —— 企业客户中，这与微软的云服务产品形成了高度契合。因此，即便估值已高，投资 Anthropic 也能让微软（以及英伟达）在人工智能持续主流化的浪潮中把握先机、从中受益。
...
我们的看法是：撇开各种商业上的合纵连横，开发者们如今越来越能随心所欲了 —— 在他们中意的云平台上，使用他们心仪的模型。这对于所有厌恶被单一供应商「绑定」的人来说，无疑是个好消息。
...
一家 AI 音乐生成初创公司结束了秘密运营状态，并获得了授权许可，得以使用全球最大几家唱片公司旗下的音乐作品来训练其生成式 AI（Generative AI）模型。
...
新进展：总部位于洛杉矶的 Klay Vision 公司成为首家与三大主流唱片公司 —— 索尼音乐娱乐（SME）、环球音乐集团（UMG）和华纳音乐集团（WMG)—— 以及拥有对应音乐作品词曲版权的出版公司，全面达成授权协议的 AI 公司。协议的具体财务条款尚未披露，根据协议，Klay 获权使用这些公司拥有版权的音乐来训练生成式模型（Generative Model）。这家初创公司计划推出一个订阅制流媒体平台，让听众能够对现有音乐进行个性化定制，同时确保版权方获得相应补偿。此外，公司还计划与独立的唱片公司、出版商、音乐人及词曲作者签订类似的合作协议。
...
工作原理：与根据文本提示创作原创音乐的音乐生成模型不同，Klay 的系统允许用户以交互方式修改已有的录音，例如调整其混音效果或音乐风格。该公司将这种交互模式称为「主动聆听（active listening）」。
...
* Klay 正在开发一个仅使用经过授权的录音进行训练的模型。该公司未透露该模型的具体构建方式或其性能细节。此外，Klay 还开发了一套归因系统，能够追溯并识别出对模型生成结果有贡献的原始录音，从而得以向相应的版权所有者支付报酬。
...
* 支付方式很可能将按每次播放（per-stream）来结算。据《金融时报》报道，在近期环球音乐集团（UMG）、华纳音乐集团（WMG）等唱片公司与 Klay、Suno、Udio、ElevenLabs、Stability AI 等 AI 初创公司的谈判中，唱片公司力推的是类似流媒体服务那种按播放次数付费的模式，而非一次性付清的许可费。
...
* Klay 的领导团队融合了人工智能（AI）领域的专业资历、唱片业的深厚经验以及数字音乐发行的实战背景。其成员包括：曾为 DeepMind 的 Lyria 音乐生成模型做出贡献的 Björn Winckler；前索尼音乐娱乐（Sony Music Entertainment，SME）总裁 Thomas Hesse；以及 Brian Whitman，他创立的音乐数据初创公司被 Spotify 收购后，他本人也加入了 Spotify 并担任首席科学家。
...
新闻背后的故事：Klay 与音乐产业巨头的这项合作，其背景是长达数年的法律纠纷。在此期间，版权方多次因版权侵权问题对 AI 公司提起诉讼。
...
* Klay 公司成立于 2021 年。其首席执行官 Ary Attie 表示，公司创立之初便「致力于赢得艺术家和词曲作者的信任」。2024 年 10 月，环球音乐集团（UMG）宣布与 Klay 达成一项「战略合作」。随后，Klay 利用接下来的一年时间，着手构建了一套许可框架。该框架旨在让艺术家、唱片公司和音乐出版商能够掌控其知识产权如何被 AI 模型使用，并针对那些基于他们作品训练的 AI 模型所生成的音乐，向这些权利人提供补偿。
...
* 2023 年，人工智能（AI）技术涌入了主流音乐圈。粉丝们利用 AI 克隆了 Drake、The Weeknd、Oasis、Eminem 和 The Beach Boys 等歌手的嗓音，制作出了这些歌手本人从未唱过的歌曲录音。实验性流行艺术家 Grimes 则顺势而为，允许她的粉丝在自己的作品中自由使用她的声音。
...
* 2024 年，初创公司 Suno 和 Udio 推出了面向大众的服务，用户只需一个网页浏览器即可使用其文生音乐（Text-to-Music）功能。根据用户输入的提示词 —— 这些提示词描述了想要的歌曲风格、主题及其他属性 —— 它们的服务能够生成几乎涵盖所有风格的完整歌曲，包括歌词。
...
* 去年，SME、UMG 和 WMG 对提供在线音乐生成服务的初创公司 Suno 和 Udio 提起了诉讼，指控这两家公司侵犯了其知识产权。
...
* 2025 年夏天，一个名为 Velvet Sundown 的虚构乐队在 Spotify 上收获了超过 50 万次播放量。上传者并未说明这些音乐是由 AI 生成的，但网络侦探们根据生成式输出中典型的人工痕迹，识破了这一骗局。
...
* 十一月中旬，环球音乐集团（UMG）和华纳音乐集团（WMG）与 AI 音乐公司 Udio 达成和解。Udio 同意停止提供其生成音乐的下载服务，转而构建自己的流媒体平台。此外，Udio 还与 Stability AI 建立了合作关系，共同为专业音乐人、词曲作者和制作人开发 AI 驱动的工具。本周，华纳音乐集团（WMG）也与另一家 AI 音乐公司 Suno 达成了和解。然而，索尼音乐娱乐（SME）和环球音乐集团（UMG）对 Suno 提起的诉讼目前仍在进行中。
...
其重要性在于：AI 生成的音乐市场仍在成形阶段，但从目前的发展来看，其前景十分广阔。目前，Suno 的目标是开拓生成音乐的市场，其前提是使用受版权保护的录音训练 AI 系统属于合理使用，但这需要法院的裁决或相关法律的修改才能最终确认。Klay 的策略则与此形成了鲜明对比。相反，Klay 专注于获取授权并补偿版权所有者，这不仅使其获得了免于版权侵权索赔的法律保护，也赢得了音乐行业的良好声誉和支持。
...
我们的想法是：音乐生成领域的先驱者与 Klay 之间的区别，让人不禁想起 2000 年左右的情景。当时，一家名为 Napster 的初创公司为乐迷提供了分享音乐文件的途径，并声称此举属于合理使用（fair use）。随后，苹果公司在 2001 年推出了 iTunes，这是一项对行业友好的音乐分发服务，提供了一个合法的替代方案。iTunes 让听众能更轻松地听到想听的音乐，为版权所有者带来了收入，因而获得了音乐产业的欢迎。同样地，Klay 的目标是为音乐产业提供一种从生成式音乐中盈利的途径，这种方式旨在补充，而非蚕食其现有的业务。
...
大语言模型（Large Language Model）在微调过程中，可能会形成类似「开朗」或「迎合」这样的特质。研究人员现已开发出一种方法，能够识别、监测并调控这些特质。
...
最新发现：来自 Anthropic、德州大学奥斯汀分校、加州大学伯克利分校，以及 AI 安全实验室 Constellation 与 Truthful AI 的研究人员 Runjin Chen 及其同事，识别出了一种**角色向量（persona vectors）**。这是大语言模型内部层输出中与特定性格特征相对应的一种模式。他们开发了一套自动化流程，能够依据简单的自然语言描述，来削弱或增强这些向量。
...
核心思路：模型在处理一批具有某种共同特质（例如「邪恶」）的样本时，对其某一特定层的输出进行平均，就能得到该特质的表征（同时也会包含这些输出共有的其他信息，比如特定的语言或句式）。为了得到纯粹的特质表征，可以从其对立特质的平均表征中减去该特质的平均表征（以此剔除共性部分）。这样得到的表征向量，就能作为一个「调节旋钮」来操控模型的倾向性。例如，在模型生成内容的过程中，将这个向量加到其内部状态上，就能强化该特质；反之，减去它则会弱化该特质。
...
工作原理：作者构建的处理流程以一个特征（trait）作为输入，并从一个目标大语言模型（Large Language Model，LLM）—— 具体指 Qwen2.5-7B 或 Llama-3.1-8B —— 计算出对应的角色向量（persona vector）。
...
* 在给定某个特质（例如邪恶）的名称及其描述（"邪恶指的是对他人表现出明显、有害意图的行为……"）后，Claude 3.7 Sonnet 生成了一组对立的系统提示，分别强调该特质及其反面（例如，"你是一个邪恶的 AI」和「你是一个乐于助人的 AI"）。此外，它还生成了 40 个中性问题（例如，"你如何看待对待动物的方式？"），用以引出能够反映所设定特质的回答。
...
*针对每一组对比的系统提示和一个问题，目标大语言模型生成了 10 个回答。作者计算了那些表现出特定立场（例如，"他们应该受苦并死去"）的回答的平均表征，与那些表现出相反立场（例如，"我们应该以善意对待他们"）的回答的平均表征之间的差值。他们将这个差值称为**角色向量（persona vector)**。
...
结果：作者们提取了三种特质对应的角色向量：邪恶、谄媚以及幻觉倾向。他们利用这些角色向量测试了三个方面：系统提示能在多大程度上诱发这些特质；这些向量能在多大程度上操控大语言模型的行为；以及它们能在多大程度上预测，针对某个特定数据集进行微调后，大语言模型在某一特质上的表现变化。他们使用 GPT-4.1-mini 来量化大语言模型的**特质表现**，即一个用于评估特定特质在模型回答中显现强度的分数。
...
* 他们通过选择模型的某一层，并比较该层在处理完最后一个提示 Token 后的输出与预设的角色向量，来监测提示所引发的行为变化。总体而言，他们发现这两个向量越相似，模型所表现出的对应特质就越强。
...
* 他们通过在模型某一层的输出上增加或减去角色向量（persona vectors）来引导大语言模型的生成行为，从而放大或抑制特定特质。在推理阶段减去角色向量后，他们不仅成功降低了特质的平均表达水平，还导致模型在 MMLU 基准测试上的表现下降。然而，当在微调阶段添加角色向量时，大语言模型的特质表达降低了，同时 MMLU 性能并未受损。在微调时进行「加法」而非「减法」，实质上阻止了大语言模型为了提升性能而去学习生成与角色向量更相似的向量。
...
* 为了预测微调数据将如何影响大语言模型（LLM）的特质表达，作者们比较了模型在微调前，在 8 个微调数据集上产生的响应与真实答案（ground truth）。具体方法是：首先，让大语言模型针对微调数据生成响应，并记录处理这些响应时某一特定神经网络层的输出。同时，也记录下模型处理真实答案时同一层的输出。接着，计算这两种输出之间的差异，并衡量该差异与角色向量（persona vector）的相似度。这个相似度越高，就表明该微调数据在后续的微调过程中，会越显著地增强大语言模型相应的特质表达。
...
为什么这很重要：这项研究为机器学习工程师提供了一个工具，能够主动管理大语言模型（LLM）的角色设定。工程师们无需等到微调完成后，才惊讶地发现模型变得一味迎合；相反，他们可以事先利用角色向量（persona vectors）来筛查微调数据，从而标识出那些可能导致不良角色偏移的整个数据集或个别数据样本。这使得微调过程变得更具可预测性 —— 因为可以预先评估可能的角色变化 —— 同时也让模型的输出结果更为安全可靠。
...
我们的思路是：利用大语言模型（LLM）将人格特质编码为向量，这为调整大语言模型的人格提供了一种工具。该发现表明，大语言模型内部的高层次行为倾向，可能也具备结构化的特征，并且是可编辑的。
